...
class XsValSchema (XsValBase):
    ...
    def validate (self, inputTree, xsdTree):
        ...
        tns = self.inputRoot.getAttributeOrDefault("targetNamespace", None)
        self._updateLookupTables (self.inputRoot, self.xsdLookupDict, tns)
...
#                self._addWarning ("Element has no type attribute and no type definition!", elementNode)
    ...
    def _checkAttributeNodesSecondLevel(self):
        ...
        for attributeNode in attributeNodes:
            ...
            self._checkNodeId(attributeNode)
    ...
    def _checkComplexTypesSecondLevel(self):
        ...
        for contentNode in contentNodes:
            ...
            if baseNsName != (XSD_NAMESPACE, "anyType"):
                if contentNode.getLocalName() == "restriction":
                    typeNsName = (XSD_NAMESPACE, "complexType")
                else:
                    typeNsName = (XSD_NAMESPACE, "simpleContent")
        ...
        if not dict.has_key(baseNsName) and baseNsName != (XSD_NAMESPACE, "anySimpleType"):
            ...
            if typeNsName == (XSD_NAMESPACE, "simpleContent"):
                if (baseNsName == (XSD_NAMESPACE, "anySimpleType") or
                    ...
                    self._addError ("Referred type '%s' must be a simple type or a complex type with simple content!" %(str(baseNsName)), node)
            else:
    ...
    def _checkNodeId (self, node):
        if node.hasAttribute("id"):
            nodeId = node["id"]
            if not self.xsdIdDict.has_key(nodeId):
                self.xsdIdDict[nodeId] = node
            else:
                self._addError ("There are multiple occurences of ID value '%s'!" %(nodeId), node)
